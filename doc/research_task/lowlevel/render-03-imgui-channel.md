...
> ⚠️ **修正（2026-08-27，dl-01）**：① 本文「CEF」表述有误——webview/video 内核实为 **miniblink**（render-05 §1 字符串实证），CEF/WebView2 非游戏内主路径；② 「video file:// 被拦」「线上会崩」等保守结论已被后续实证收窄——https mp4 线上真局播完（render-12），完整视频生产方案（pak 提取 + file:// + 双轨音频 + 三端实测）见 [pak-io-native.md](../research/pak-io-native.md)；③ 「webview run_js/web_message 双向通信未实测」遗留项已完成——**lua↔JS 双向桥全通**（imgui 通道需手动登记 base.ui.map + register_event），见 [webview-bridge.md](../research/webview-bridge.md)。
...
- webview 内容出现在 **游戏视口 WGC 截图**里 → webview 是**引擎内嵌渲染**（离屏 miniblink 合成进游戏 UI 纹理【2026-08-27 修正：原记 WebView2 有误】，不是独立 HWND overlay）。
- video = webview + HTML5 `<video>`（GUIVideo，strings 452635-452640 内嵌 HTML 模板实证）；file:// 协议被拦（媒体不加载）【2026-08-27 修正：PC webview html 文本加载时 file:// 视频被拦属实，但解法是播放器 HTML 写成文件 + `url='file://...'` 加载，iOS/PC 三端已实测可播，见 pak-io-native.md §7】，data:/http(s) 待测；`run_js` 属性可注入 JS（play() 等）。
...
5. **视频** → 机制打通（imgui video 控件渲染播放器）；file:// 被 CEF【修正：miniblink】拦，http(s)/data: 待测；~~注意 bench 注释称 video 线上（tester）会崩——生产慎用，需线上验证~~ → **2026-08-24 render-12 证伪**（线上真局 https mp4 播完不崩）；完整生产视频方案见 [pak-io-native.md](../research/pak-io-native.md)（三端实测）。

## 5. 遗留探针（下轮）

- ~~imgui video 用 http(s) mp4 测试实际播放~~ → **2026-08-24 U28 实证成功**（见 §7）。
- ~~webview run_js/web_message 双向通信实测（lua↔JS 桥）~~ → **2026-08-26 完成**：双向桥全通（[webview-bridge.md](../research/webview-bridge.md) §2/§3；imgui 通道手动登记三步法）。canvas2d 离屏渲染评估。
...
- video 控件 file://：CEF【修正：miniblink】安全策略拦截本地文件（播放器框出、0:00 不加载）；生产解法 = 播放器 HTML 写成文件 + file:// 加载 + 双轨音频（pak-io-native.md §7）。~~video 线上崩溃风险见 bench 注释，生产不建议依赖~~ → render-12 已证伪。
