...
5. **update-info 不覆盖的资产**：`ui/`（游戏字体族 ui/font/regular 等）、`characters/`、`effect/`、`anim/` 等编辑器基座资源——任何 list 组合都不返回。这些只能来自编辑器安装包 → **0.2.0 决策：打成 base_assets.7z 随我们自己的 release 分发**（稳定很少变）。
